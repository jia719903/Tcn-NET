...
class FullModel(nn.Module):
    ...
    def __init__(self, model, sem_loss, bd_loss ,dice_loss):  # 添加dice_loss参数
        super(FullModel, self).__init__()
        self.model = model
        self.sem_loss = sem_loss
        self.bd_loss = bd_loss
        self.dice_loss = dice_loss  # 存储Dice Loss
    ...
    def forward(self, inputs, labels, bd_gt, *args, **kwargs):
        ...
        # expanded_labels = expanded_labels.expand(-1, 2, -1, -1)
        squeezed_tensor = outputs[-1][:, 0, :, :]
        # print(squeezed_tensor.shape)
        # print(labels.shape)
        loss_dice = self.dice_loss(squeezed_tensor,bd_gt)
        ...
        # loss = loss_s + loss_b + 0.005*loss_dice  # 将Dice Loss添加到总损失中
        loss = loss_s + loss_b + loss_sb+loss_dice
        # loss = loss_s + loss_b + loss_sb
        # return torch.unsqueeze(loss, 0), outputs[:-1], acc, [loss_s, loss_b, loss_dice]  # 返回的损失列表中也包含了Dice Loss
